...
class MinkContext extends RawMinkContext implements TranslatableContext
{
    ...
    /**
     * Autocomleting for the form elegilibility
     *
     */

    public function autocompleteForm(TableNode $table, $element, $autocompleteElement, $formInput)
    {
        $page = $this->getSession()->getPage();
        $hash = $table->getHash();
        foreach ($hash as $row) {
            $page->fillField($element, $row[$formInput]);
            $page->findById($element)->click();
            $this->getSession()->wait(10000, '(0 === jQuery.active)');
            $results = $page->findAll('css', $autocompleteElement);
    ...
    {
        $this->spin(function(MinkContext $context) use ($cssSelector) {
            /** @var $context */
